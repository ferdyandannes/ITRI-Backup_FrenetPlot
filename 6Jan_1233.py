...
def gambar_posisi(info):
    ...
    return bxmin, bxmax, bymin, bymax, nomor_frame, pos_x, pos_y
...
def viz_frenet(data_dir):
    ...
    # Second pseudo car
    position_path2 = os.path.join(data_dir,"pseudo2.txt")

    with open(position_path2) as position2:
        position_info2 = position2.readlines()

    lines2 = len(position_info2)
    ...
    for line in range(lines):
        ...
        # Blank images for drawing the object position
        ...
        # Plot first pseudo-car ###################################################
        print("first")
        info = position_info[line].strip().split()
        bxmin, bxmax, bymin, bymax, nomor_frame, pos_x, pos_y = gambar_posisi(info)
        pos_x_list.append(pos_x)
        pos_y_list.append(pos_y)
        cv2.rectangle(img_bird,(bxmin, bymin),(bxmax, bymax),(195,6,147),-1)
        
        # Plot second pseudo-car ###################################################
        print("second")
        info2 = position_info2[line].strip().split()
        bxmin, bxmax, bymin, bymax, _, pos_x, pos_y = gambar_posisi(info2)
        pos_x_list.append(pos_x)
        pos_y_list.append(pos_y)
        cv2.rectangle(img_bird,(bxmin, bymin),(bxmax, bymax),(195,126,147),-1)
        ...
        print("pos_x_list = ", pos_x_list)
        print("pos_y_list = ", pos_y_list)

        info_file = h5py.File(BEV_dir+nomor_frame+'.h5','w')
        info_file.create_dataset('frame_id', data = nomor_frame)
        info_file.create_dataset('pos_x', data = pos_x_list)
        info_file.create_dataset('pos_y', data = pos_y_list)
...
def viz_frenet2(data_dir):
    ...
    for line in range(lines):
        ...
        info = position_info[line].strip().split()
        bxmin, bxmax, bymin, bymax, nomor_frame, pos_x, pos_y = gambar_posisi(info)
        # Image for drawing (Only after first pseudo)
...
def viz_front(data_dir):
    ...
    for frame_id in range(len(images)-1):
        ...
        for i in range(len(pos_x)):
            ...
            lane_color = trajectories_color(int(i))
            cv2.rectangle(mask, (int(post_x) - 17, int(post_y) - 30), (int(post_x) + 17, int(post_y) + 10), lane_color, cv2.FILLED)
            ...
            if i == 0:
                ...
                cover_image = cv2.addWeighted(cover_image, 1, inverse_img, 0.3, 0)
